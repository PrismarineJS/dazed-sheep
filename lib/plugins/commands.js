  ...
  player.commands.add({
   ...
   op: true,
    action(params) {
      ...
      var reason = 'You have been kicked from the server';
      
      if(params.toString().split(' ')[1] != null) {
        reason = params.toString().split(' ')[1].trim();
      }
...
module.exports.server = function(serv) {
  ...
  serv.handleCommand = function(str) {
    try {
      const res = serv.commands.use(str, true);
      if (res)
        serv.log.info(serv.cleanCommandOutput(res));
    }
    catch(err) {
      if (err)
        serv.log.info('Error: ' + err.message);
  ...
  serv.cleanCommandOutput = function(str) {
    return str.replace(/\&\w/ig, '').replace(/\//ig, '');
  }
  ...
  serv.commands.add({
    ...
    action: function action(_ref) {
      ...
      if (page < 0) return 'Page # must be >= 1';
      var hash = serv.commands.uniqueHash;
      ...
      if (found.length == 0) {
        ...
        if (found.indexOf('search') != -1) {
          var baseCmd = hash[search];
          serv.log.info(baseCmd.base + ' -' + (baseCmd.params && baseCmd.params.info && ' ' + baseCmd.params.info || ''));
        } else {
          serv.log.info('--- Showing help page ' + (page + 1) + ' of ' + totalPages + ' (/help <page>) ---');
        }
        for (var i = PAGE_LENGTH * page; i < Math.min(PAGE_LENGTH * (page + 1), found.length); i++) {
          if (found[i] === search) continue;
          var cmd = hash[found[i]];
